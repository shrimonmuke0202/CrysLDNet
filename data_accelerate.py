...
from graphs_pddformer import PygGraph, PygStructureDataset, chemical_symbols
...
class ConstructTarget():
    ...
    def forward(self, i):
        ...
        i['coords'] = pd_dict['position_gt']
        
        pd_dict['lattice_gt'] = torch.tensor(np.array(i['lattice_mat']))
        
        i['lattice_mat'] =  pd_dict['lattice_gt']
        pd_dict['atoms'] = i
...
def get_pyg_dataset(
    ...
    if target == "shear modulus" or target == "bulk modulus":
        val_list = [vals[i].item() for i in range(len(vals))]
        vals = val_list
    output_dir = "./saved_data/" + tmp_name + "graph_angle.pkl" # for fast test use
    print("data range", np.max(vals), np.min(vals))
    print(output_dir)
    if now:
...
def get_train_val_loaders(
    ...
    if (
        ...
        position_gt = []
        lattice_gt = []
        construct = ConstructTarget(mask_ratio=mask_ratio)
        ...
        if pre_train:
            ...
            data_frame_temp = pd.DataFrame(df['atoms'].progress_apply(construct.forward))
